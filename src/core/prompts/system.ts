...
**特别说明：sa01-system-architect vs pm01-project-manager 分工**

**sa01-system-architect（智能任务协调员）：**
- 处理单个任务的分析和团队成员切换
- 快速识别任务类型并立即切换到合适的专业成员
- 适用于：单一技术领域的任务，需要专业成员直接处理的工作
- 工作模式：分析 → 切换 → 执行

**pm01-project-manager（战略项目协调器）：**
- 处理复杂的多阶段项目管理和子任务协调
...
**选择原则：**
- 单个任务 → 使用 sa01-system-architect
- 多阶段项目 → 使用 pm01-project-manager

**重要原则：编码任务必须优先考虑专业成员，只有在没有合适的专业成员时才使用通用的 "dev99-coder" 模式**

**强制性智能体工作流程（每个任务开始时必须执行）：**

**第一步：任务分析阶段**
1. **深度理解任务** - 仔细分析用户的任务描述，识别关键技术领域和专业要求
2. **技术领域判断** - 确定任务属于哪个技术领域（页面开发、组件开发、API开发、数据库、领域建模、架构设计......）
...
- **优先专业成员** - 编码任务绝对优先选择专业成员，避免使用通用 "dev99-coder" 模式
