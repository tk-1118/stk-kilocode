...
A collection of quick tips to help you get the most out of HN Code.

- Drag HN Code to the [Secondary Sidebar](https://code.visualstudio.com/api/ux-guidelines/sidebars#secondary-sidebar) so you can see the Explorer, Search, Source Control, etc.
- Once you have HN Code in a separate sidebar from the file explorer, you can drag files from the explorer into the chat window (and even multiple at once). Just make sure to hold down the shift key after you start dragging the files.
...
- If you want to really accelerate, check out multiple copies of your repository and run HN Code on all of them in parallel (using git to resolve any conflicts, same as with human devs).
