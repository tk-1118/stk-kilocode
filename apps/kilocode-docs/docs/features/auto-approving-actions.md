...
> ⚠️ **SECURITY WARNING:** Auto-approve settings bypass confirmation prompts, giving HN Code direct access to your system. This can result in **data loss, file corruption, or worse**. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
...
2. Select which actions HN Code can perform without asking permission
...
| ------------------------------ | ------------------------------------------------ | ----------- |
| **Read files and directories** | Lets HN Code access files without asking         | Medium      |
| **Edit files**                 | Lets HN Code modify files without asking         | **High**    |
...
| **Use MCP servers**            | Lets HN Code use configured MCP services         | Medium-High |
| **Switch modes**               | Changes between HN Code modes automatically      | Low         |
...
> **Allow HN Code to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.**
...
**Description:** "When enabled, HN Code will automatically view directory contents and read files without requiring you to click the Approve button."

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files HN Code can access.
:::
...
This setting allows HN Code to modify your files without confirmation. The delay timer is crucial:
...
Allows HN Code to control a headless browser without confirmation. This can include:
...
This setting automatically retries API calls when they fail. The delay controls how long HN Code waits before trying again:
...
Allows HN Code to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
:::
...
Enables HN Code to create and complete subtasks automatically. This relates to workflow organization rather than system access.
:::
