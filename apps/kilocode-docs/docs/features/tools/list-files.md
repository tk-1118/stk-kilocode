...
The `list_files` tool displays the files and directories within a specified location. It helps HN Code understand your project structure and navigate your codebase effectively.
...
- When HN Code needs to understand your project structure
- When HN Code explores what files are available before reading specific ones
- When HN Code maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When HN Code needs to check for specific file types (like configuration files) across a project
...
- When starting a new task, HN Code may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), HN Code first lists directories to know where to look.
- When providing recommendations for code organization, HN Code examines the current project structure first.
- When setting up a new feature, HN Code lists related directories to understand the project conventions.
