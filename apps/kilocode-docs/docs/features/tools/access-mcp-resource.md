...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows HN Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When HN Code needs additional context from external systems
- When HN Code needs to access domain-specific data from specialized MCP servers
- When HN Code needs to retrieve reference documentation hosted by MCP servers
- When HN Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
    ...
    - Returns the processed resource data to HN Code for use in the current task
...
- When helping with API development, HN Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, HN Code accesses current data samples from connected MCP servers.
- When working in specialized domains, HN Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, HN Code references compliance requirements from documentation resources.
