...
- When HN Code needs to find code related to specific functionality across your project
...
- When implementing a new feature, HN Code searches for "authentication middleware" to understand existing patterns before writing new code.
- When debugging an issue, HN Code searches for "error handling in API calls" to find related error patterns across the codebase.
- When refactoring code, HN Code searches for "database transaction patterns" to ensure consistency across all database operations.
- When onboarding to a new codebase, HN Code searches for "configuration loading" to understand how the application bootstraps.
